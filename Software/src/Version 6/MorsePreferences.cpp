...
parameter MorsePreferences::pliste[] = {
    ...
    {0, 0, 13, 1, // Offset for LICW Carousel
     ...
     true,
     {"BC1: r e a", "BC1: t i n", "BC1: p g s", "BC1: l c d", "BC1: h o f", "BC1: u w b", "BC2: k m y", "BC2: 5 9 ,", "BC2: q x v", "BC2: ar sk =", "BC2: 1 6 .", "BC2: z j /", "BC2: 2 8 bk", "BC2: 4 0"}},
...
boolean MorsePreferences::setupPreferences(uint8_t atMenu)
{
  ...
  while (true)
  { // we wait for single click = selection or long click = exit - or single or long click or RED button, or for a serial event
    serialEvent();
    if (goToMenu)
    {
...
{
  String valueLine;
  valueLine.reserve(20);
  const String emptyLine = "                    ";
  const int maxLength = 14;
  String jsonValueLine;
  jsonValueLine.reserve(48);
  String item;
  item.reserve(18);
  int value;
...
int MorsePreferences::getValue(prefPos pos)
{ /// a value to return for m32protocol
  switch (pos)
  {
  case posKochFilter:
...
boolean MorsePreferences::adjustKeyerPreference(prefPos pos)
{ /// rotating the encoder changes the value, click returns to preferences menu
  ...
  { // we wait for single click = selection or long click = exit
    ...
    {                                                          /// t == 1 or -1
      ...
      if (pos <= posSerialOut)
      { /// "normal" procedure through preferences menu
      ...
      { /// "strange" way of adjusting, outside preferences menu
        switch (pos)
        {
        case posKochFilter:
        ...
        case posLoraBand:
          ...
          case 0:
            ...
            break;
          case 1:
            MorsePreferences::loraQRG = QRG866;
            break;
          case 2:
            MorsePreferences::loraQRG = QRG920;
            break;
          }
          break;
        case posLoraQRG:
          ...
          {
          case 0:
            MorsePreferences::loraQRG = constrain(MorsePreferences::loraQRG, 433.65E6, 434.55E6);
            break;
          case 1:
            MorsePreferences::loraQRG = constrain(MorsePreferences::loraQRG, 866.25E6, 869.45E6);
            break;
          case 2:
            MorsePreferences::loraQRG = constrain(MorsePreferences::loraQRG, 920.25E6, 923.15E6);
            break;
          }
          break;
        case posSnapRecall:
...
{ // temp is a bitmap, 1 byte long, with a bit set for each existing snapshot
  uint8_t t = temp;
...
void MorsePreferences::determineBoardVersion()
{
  ...
  delay(1000);
  if (MorsePreferences::boardVersion == 0)
  {                         // no board version had been set previously, so we determine and set it here
...
void MorsePreferences::setCwMem(uint8_t number, String value)
{ // store CW memories in preferences
...
void Koch::createWords(uint8_t maxl, uint8_t koch)
...
void Koch::createAbbr(uint8_t maxl, uint8_t koch)
{ // this function creates an array of abbrevs that are compliant to Koch filter and max word length
  numberOfAbbr = 0;
  maxl = (maxl == 0 ? 0 : maxl + 1);
...
{ /// returns the highest Koch sequence number of a word; if it is not in the charset, return a high number
  uint8_t thisKoch = 0;
  int index;
...
void Koch::setup()
{ // create the Koch tables according to custom char or Koch filter, and length
  if (MorsePreferences::useCustomChars)
...
void Koch::setKochChars(uint8_t sequence)
{ // define the demanded Koch character set: Koch sequenze: 0 = native/JLMC, 1 = LCWO, 2 = CW Academy, 3 = LICW, 4 = Custom
  switch (sequence)
  {
  case 1:
  ...
  case 2:
    ...
    break;
  case 3:
    setupLICWkochChars(MorsePreferences::pliste[posCarouselStart].value);
...
uint8_t Koch::setupLICWkochChars(uint8_t start)
{ // set up the string of characters used for LICW carousel Koch training, return length
...
void Koch::setCustomChars(String chars)
{ // define the custom character set
  MorsePreferences::customCharSet = chars;
}

String Koch::getNewChar()
{ // for Koch Learn New Character
  return String(kochCharSet.charAt(MorsePreferences::kochFilter - 1));
}

String Koch::getKochChar(uint8_t i)
{ // get a String consisting of a single character at pos i in kochCharSet (i starting with 0)
  return String(kochCharSet.charAt(i));
}

String Koch::getRandomChar(int maxl)
{ // get a random character word for Koch, with max length maxl
  String result = "";
  result.reserve(7);
...
{ // get a random english word for Koch (preselected with max length and Koch filter (or custom character filter)
  if (numberOfWords == 0)
