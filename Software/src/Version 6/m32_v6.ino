...
void setup()
{
  ...
  { // while the SPIFFS has not been initialized (i.e. 1st programming), we are not going to check the key press
    if (key_was_pressed_at_start())
    {
      ...
      case 1:
        ...
        break;
      case 2:
        MorsePreferences::loraSystemSetup();
        break;
      default:
        break;
      }
...
void loop()
{
  ...
  case morseGenerator:
    ...
    { /// no paddle pressed - check stop flag
      checkStopFlag();
    }
    if (genIsActive)
      generateCW();
  ...
  case -1:
    ...
    break;
  case 2:
    MorseOutput::decreaseBrightness(); // step through screen brightness levels
    break;
  }
  ...
  case 1:
    ...
    { // when Keyer is active, we select a keyer memory
      if (m32protocol)
...
boolean doPaddleIambic(boolean dit, boolean dah)
{
  ...
  case DIT:
    ...
    case IAMBICB:
      curtistimer = 2 + (ditLength * MorsePreferences::pliste[posCurtisBDotTiming].value / 100);
  ...
  case DAH:
    ...
    case IAMBICB:
      curtistimer = 2 + (dahLength * MorsePreferences::pliste[posCurtisBDahTiming].value / 100); // enhanced Curtis mode B starts checking after some time
      break;
    case NONSQUEEZE:
      curtistimer = 3;
      break;
...
uint8_t readSensors(int left, int right, boolean init)
{ // variables left and right designate Pin numbers - not relevant for load sensor
  uint8_t v, lValue, rValue;
  int32_t LPressure;
  int32_t RPressure;
  // at startup we have collected idle values (read01 and read02) from adc1 and adc2 sensor
...
{ //// give me a random CW abbreviation , max maxLength chars long (1-5 = 2-6) - 0 returns any length
  if (maxLength > 5)
    maxLength = 0;
  else if (maxLength != 0)
    ++maxLength;
...
{                                               // random call-sign like pattern, maxLength = 1-4 (=3-6), 0 returns any length
  ...
  case 2:
    ...
    break;
  case 3:
    call += CWchars[random(26, 23)]; // CWchars.charAt(random(26,36));
    call += CWchars[random(0, 26)];  // CWchars.charAt(random(0,26));
    l = 2;
    break;
...
{ ////// this is called from loop() (frequently!)  and generates CW  ////////////////////////////
  ...
  case KEY_DOWN:
    ...
    if (!CWword.length())
    { // we just ended the the word, ...  //// intercept here in Echo Trainer mode or autoStop mode
...
unsigned int wordDoublerICS()
{
  ...
  case 3:
    ...
    break;
  default:
    return interCharacterSpace; // case 1:
    break;
  }
}

int pitch()
{ // find out which pitch to use for the generated CW tone
...
void fetchNewWord()
{
  ...
  { //// playCW  //// - get next word from m32protocol input buffer
  ...
  else
  {
    ...
    else if (morseState == echoTrainer)
    {
      ...
      {
      case REPEAT_WORD:
        if (MorsePreferences::pliste[posEchoRepeats].value == 7 || repeats <= MorsePreferences::pliste[posEchoRepeats].value)
          clearText = echoTrainerWord;
        else
        {
          clearText = echoTrainerWord;
    ...
    randomGenerate:
      ...
      if (clearText != "+")
      {
        ...
        {
        case RANDOMS:
          clearText = getRandomChars(MorsePreferences::pliste[posRandomLength].value, MorsePreferences::pliste[posRandomOption].value);
          break;
        case CALLS:
          clearText = getRandomCall(MorsePreferences::pliste[posCallLength].value);
          break;
        case ABBREVS:
          clearText = getRandomAbbrev(MorsePreferences::pliste[posAbbrevLength].value);
          break;
        case WORDS:
          clearText = getRandomWord(MorsePreferences::pliste[posWordLength].value);
          break;
        case KOCH_LEARN:
          clearText = koch.getNewChar();
          break;
        case MIXED:
          rv = random(4);
          switch (rv)
        ...
        case KOCH_MIXED:
          ...
          case 1:
            ...
            break;
          case 2:
            clearText = getRandomChars(MorsePreferences::pliste[posRandomLength].value, OPT_KOCH); // Koch option!
            break;
          }
          break;
        case KOCH_ADAPTIVE:
          clearText = getRandomChars(MorsePreferences::pliste[posRandomLength].value, OPT_KOCH_ADAPTIVE);
          break;
        case PLAYER:
          if (MorsePreferences::pliste[posRandomFile].value != 0)
...
{                                  /// symbol to be displayed on status line
...
{ /// measure battery voltage and return result in milliVolts
...
void cwForTx(int element)
{
  ...
  if (pairCounter == 0)
  { // we start a brand new word for LoRA/Wifi - clear buffer and set speed first
...
void sendWithWifi()
{ // hand this string over as payload to the WiFi transceiver
  // send packet
...
void onLoraReceive(int packetSize)
{
  ...
  { // 1st actual morse element must not be 0b00
  ...
  if (reason == "")
    reason = "EOFC";
error:
  DEBUG("Invalid LoRa Packet: " + reason + "! Discarded...");
}
...
endloop:
  if (i < (l - 1))
  { // we found EOW, but the packet continues! must be a spurious packet!
    *cwword = "";
    return 0;
  }
...
void keyOut(boolean on, boolean fromHere, int f, int volume)
{
  ...
  case 0: // noTx = true;                            // par "Key Ext Tx" = NEVER //true has already been set
    break;
  case 1:
    if ((morseState == morseKeyer || morseState == morseTrx) && fromHere) //  CW Keyer Only (also CW Trx)
      noTx = false;
    break;
  case 2:
    if ((morseState == morseKeyer || morseState == morseGenerator) && fromHere) // CW Keyer && generator
      noTx = false;
    break;
  case 3:
    if (((morseState == morseKeyer || morseState == morseGenerator || morseState == morseTrx) && fromHere) ||
...
void audioLevelAdjust()
{
  ...
  while (true)
  { // we also key the transmitter (can be used for tuning the Tx...)
    Buttons::volButton.Update();
...
void dispMem(int8_t memNo)
{
  ...
  { // exit
    ...
    if (m32protocol)
      jsonCreate("message", "Click to Exit", "");
  }
  else
  {
    ...
    if (m32protocol)
      jsonCreate("message", "Memory " + Value, "");
  }
}
...
String getWord()
{
  ...
  while (file.available())
  {
    ...
    { // end of word
      ...
      { // a comment starts here - ignore everything until end of line
        result = "";
        while (file.available())
        {
          char c = file.read();
        ...
        return result; // no comment, so return result
...
{ // all to lower case, and convert umlauts
  ...
  {                                                                                                      // disregard all non-standard characters
...
{ /// replace umtf umlauts with digraphs, and interpret pro signs, written e.g. as [kn] or <kn> or \KN
  s.replace("ä", "ae");
  s.replace("ö", "oe");
  s.replace("ü", "ue");
  s.replace("Ä", "ae");
...
void serialEvent()
{
  ...
  if (stringComplete)
  {
    ...
    else
    {
      ...
      { /// client wants to switch m32 Protocol on
        m32protocol = true;
        jsonDevice();
      }
    }
...
{ /// PUT command
  ...
  else if (type == "snapshot")
  {
    ...
    else if (token == "clear" || token == "recall")
    {
      ...
      { /// and memCounter must be > 0
        for (uint8_t y = 0; y < MorsePreferences::memCounter; ++y)
        { // we look if snapshot number i does exist within existing snapshot
          if (MorsePreferences::memories[y] == i)
          {                       // we found the correct snapshot
...
void jsonMenuList()
{ // get all parameter names and their values, and send them as json object
...
{ /// find parameter "token" and create json object for it
  String pname;
  pname.reserve(20);
  bool found = false;
...
{ // change a parameter, return true if error, false if successful
  String pname;
  pname.reserve(20);
  bool found = false;
  ...
  for (uint8_t i = 0; i <= posSerialOut; ++i)
  {
    ...
    if (v != 0)
    { // wordDoubler and Autostop are mutually exclusive!
      if (i == posWordDoubler)
      {
...
void jsonGetKoch()
{ // get current Koch lesson setting, and associated values
...
{ /// if active == 1: we are LEAVING a mode
...
void jsonFileStats()
{ // get info about SPIFFS file system
...
void jsonFileText()
{                                              // get file content as json object (file should not contain curly braces!)
  File file = SPIFFS.open("/player.txt", "r"); // Open the file for reading in SPIFFS - no error handling, file must exist
...
{ // parse str first to get 1, 2 or 3 as argument
  switch (i)
  {
  case 1:
...
void selectWifi(int i)
{
  ...
  {
  case 1:
    MorsePreferences::writeWifiInfo(MorsePreferences::wlanSSID1, MorsePreferences::wlanPassword1, MorsePreferences::wlanTRXPeer1);
    break;
  case 2:
    MorsePreferences::writeWifiInfo(MorsePreferences::wlanSSID2, MorsePreferences::wlanPassword2, MorsePreferences::wlanTRXPeer2);
    break;
  case 3:
    MorsePreferences::writeWifiInfo(MorsePreferences::wlanSSID3, MorsePreferences::wlanPassword3, MorsePreferences::wlanTRXPeer3);
    break;
  }
  jsonOK();
}
...
void stopPlayCw()
{ /// sort of emergency stop for M32p cw/play
  keyOut(false, true, 0, 0);
  playCW = false;
  repeatPlayCW = false;
